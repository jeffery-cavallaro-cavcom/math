...
\item Perform the operation:
  \[\frac{y}{y^2-81}+\frac{2}{9-y}\]

  \vspace{2in}

\item Perform the operation:
  \[\frac{16(x-2)}{x^2+10x+25}\cdot\frac{2x+10}{4x-8}\]

  \newpage

\item Perform the operation:
  \[\frac{\frac{7-2y}{2y}}{\frac{7}{y}-2}\]
...
\item Use the graph of $K(x)$ to answer the following questions:
  ...
  \begin{tikzpicture}[scale=2]
    \begin{axis}[
      ...
      \node (a) [circle,draw,scale=0.5] at (-4,1) {};
      \node (b) [circle,fill,scale=0.5] at (5,2) {};
      \draw (a) to (-2,-1) to (1,2) to (3,-2) to (b);
      \node at (3,3) {$y=K(x)$};
    \end{axis}
  \end{tikzpicture}
